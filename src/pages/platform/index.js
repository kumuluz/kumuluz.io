...
import { SideMenu, WhitepaperSection } from '../../components/sub-pages';
...
export default class TechnologyPage extends Component {
  ...
  waypointEntered(waypoint) {
    const newState = this.state;
    newState.menuItem = waypoint;
  ...
  render() {
    ...
    return (
      <div className="platform-page">
        ...
        <SideMenu
          selectedItem={this.state.menuItem}
          menuItems={menuItems(t)}
          menuHeader={<p>{t('sidebar.title')}</p>}
          onSelect={menuItem => this.menuItemSelected(menuItem)}
        >
          {this.renderdAPIsSection(t)}
          {this.renderPrebuiltdAPIsSection(t)}
          {this.renderMarketplaceSection(t)}
          {this.renderCommunitySection(t)}
  ...
  renderdAPIsSection(t) {
    ...
    'java', 'node', 'go', 'kotlin', 'python', 'cs'

    return (
      <span>
        <Waypoint onEnter={() => this.waypointEntered('dAPIs')} />
        ...
        <Waypoint onEnter={() => this.waypointEntered('security')} bottomOffset="50%" />
        <h3 id="security">{t('security.title')}</h3>
        <p>{t('security.content')}</p>
        <ul>{['protocol', 'proof', 'oauth', 'federation', 'gdpr'].map((li, key) => <li key={key}>{t(`security.points.${li}`)}</li>)}</ul>
        <Row className="mt-5">
        ...
        <Waypoint onEnter={() => this.waypointEntered('cloud-abstraction')} bottomOffset="50%" />
        ...
        <ul>
          {['cloud', 'file', 'database'].map((point, key) => (
            <li key={key} dangerouslySetInnerHTML={{ __html: t(`cloudAbstraction.solution.points.${point}`) }} />
          ))}
        </ul>
        ...
        <Waypoint onEnter={() => this.waypointEntered('dlt')} bottomOffset="50%" />
        ...
        <Waypoint onEnter={() => this.waypointEntered('seamless-integration')} bottomOffset="50%" />
  ...
  renderPrebuiltdAPIsSection(t) {
    return (
      <span>
        <Waypoint onEnter={() => this.waypointEntered('prebuilt-dAPIs')} bottomOffset="50%" />
        <h3 id="prebuilt-dAPIs" className="mt-5 mt-lg-auto" dangerouslySetInnerHTML={{ __html: t('prebuiltdAPIs.title') }} />
        ...
        <Row>
          {['userManagement', 'ccm', 'kyc'].map((cb, key) => (
            <Col key={key} xs="12" sm="6" className="mb-3 pb-3">
              <Checkbox>
                <span className="no-transform" dangerouslySetInnerHTML={{ __html: t(`prebuiltdAPIs.developed.checkboxes.${cb}`) }} />
              </Checkbox>
            </Col>
          ))}
        </Row>
        ...
        <Row>
          {[
            ...
            <Col key={key} xs="12" sm="6" xl="4" className="mb-3 pb-3">
              <Checkbox>
                <span className="no-transform" dangerouslySetInnerHTML={{ __html: t(`prebuiltdAPIs.pipeline.checkboxes.${cb}`) }} />
              </Checkbox>
            </Col>
          ))}
        </Row>
  ...
  renderMarketplaceSection(t) {
    return (
      <span>
        <Waypoint onEnter={() => this.waypointEntered('marketplace')} bottomOffset="50%" />
  ...
  renderCommunitySection(t) {
    return (
      <span>
        <Waypoint onEnter={() => this.waypointEntered('community')} bottomOffset="50%" />
  ...
  renderSmartContractsSection(t) {
    return (
      <div id="smart-contracts">
        <Waypoint onEnter={() => this.waypointEntered('smart-contracts')} bottomOffset="50%" />
        ...
        <ul>
          {['usage', 'price', 'load', 'execution', 'other'].map((p, k) => <li key={k}>{t(`smartContracts.licencing.points.${p}`)}</li>)}
        </ul>
        ...
        <ul>{['developers', 'miners'].map((p, k) => <li key={k}>{t(`smartContracts.incentive.points.${p}`)}</li>)}</ul>
